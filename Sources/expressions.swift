...
func addDigit(_ lhs: Term, _ rhs: Term, _ result: Term, _ rest: Term)-> Goal
{
    return
        (lhs === d0 && result === rhs && rest === d0) ||
            ...
            ((lhs === d1) && (rhs === d5) && (result === d6) && (rest === d0)) ||       //lhs=1
            ...
            ((lhs === d2) && (rhs === d5) && (result === d7) && (rest === d0)) ||       //lhs=2
            ...
            ((lhs === d3) && (rhs === d5) && (result === d8) && (rest === d0)) ||       //lhs=3
            ...
            ((lhs === d4) && (rhs === d5) && (result === d9) && (rest === d0)) ||       //lhs=4
            ...
            ((lhs === d5) && (rhs === d4) && (result === d9) && (rest === d0)) ||       //lhs=5
            ...
            ((lhs === d6) && (rhs === d5) && (result === d1) && (rest === d1)) ||        //lhs=6
            ...
            ((lhs === d7) && (rhs === d4) && (result === d1) && (rest === d1)) ||        //lhs=7
            ...
            ((lhs === d8) && (rhs === d5) && (result === d3) && (rest === d1)) ||       //lhs=8
            ...
            ((lhs === d9) && (rhs === d4) && (result === d3) && (rest === d1)) ||       //lhs=9
...
func evalAdd(_ lhs : Term,_ rhs : Term,_ result : Term) -> Goal {
    return
        ...
        { t in
            ...
            return reverse(lhs, lhs_inversed) && reverse(rhs, rhs_inversed) && //Inverse given operands
                evalAddSecond(lhs_inversed, rhs_inversed, result_inversed) && //Add said operands
                reverse(result_inversed, result) //Inverse the result to retreive original one
        }
}
...
func evalMinus(_ lhs : Term,_ rhs : Term,_ result : Term) -> Goal   // lhs - rhs = result <=> lhs = result + rhs
{
    return
        delayed(freshn
          { t in
...
func evalMul (_ lhs : Term,_ rhs : Term,_ result : Term) -> Goal //lhs * rhs = result <=> lhs * (rhs-1) = result - lhs
{
...
func evalDiv (_ lhs : Term,_ rhs : Term,_ result : Term) -> Goal // lhs/rhs = result <=> lhs = result * rhs
{
    return evalMul(rhs, result, lhs)
}

// compare digit and returns true or false
...
func evalLessThan(_ lhs : Term,_ rhs : Term) -> Goal
{
    return freshn
    {
      t in
        ...
        let rhead = t["rhead"]
        let rtail = t["rtail"]
        return (lhs === List.empty && rhs === List.cons(lhead,ltail)) || //lhs is empty and rhs is not
                compare(lhs, rhs, "lt") || //Only one value in each list. Is lhs < rhs ?
                ((lhs === List.cons(lhead, ltail) && rhs === List.cons(rhead, rtail)) && //Otherwise we repeat the process recursively...
...
func evalComparison (_ input: Term, _ result: Term) -> Goal //return true or false
{
    return freshn
    { t in
        let lhs = t["lhs"]
